...
io.on("connection", (socket) => {
    ...
    try {
      ...
      let color: 'red' | 'blue' | null = null;
      if (red.toString() === playerId) color = 'red';
      else if (blue.toString() === playerId) color = 'blue';
    ...
    socket.on('placePiece', async ({ gameId, piece, playerId, formedMill }, acknowledge) => {
      try {
        ...
        const updateResult = await Game.updateOne(
          { _id: gameId, "board.pieces.position": { $ne: { x: piece.position.x, y: piece.position.y } } },
          update
        );
...
socket.on('movePiece', async ({ gameId, pieceId, newPosition, playerId }, acknowledge) => {
  ...
  if (!piece) {
    ...
    console.log('Available pieces for current player:', 
      game.board.pieces.filter(p => p.player === currentPlayerColor).map(p => p.id));
    return acknowledge({ success: false, error: "Invalid piece" });
  }
